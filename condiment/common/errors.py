...
class IncompatibleVersion(CondimentError):
...
class FileNotFound(CondimentError):
...
class CharmError(CondimentError):
...
class FileAlreadyExists(CondimentError):
...
class NoConnection(CondimentError):
...
class ConstraintError(CondimentError):
...
class ProviderError(CondimentError):
...
class CannotTerminateMachine(CondimentError):
...
class InvalidPlacementPolicy(CondimentError):
...
class ServiceError(CondimentError):
    """Some problem with an upstart service"""


class SSLVerificationError(CondimentError):
...
class SSLVerificationUnsupported(CondimentError):
