...
execute (local) operations on Condiment's development environment (Docker,
...
from condiment.common.fabric.map import FABRIC_FUNCTION_MAP
...
@task
def generate_condiment_base_image_i386():
    """
    """
    env.arch = 'i386'
    env.debian_base_image = 'luisalejandro/debian-i386'
    env.condiment_base_image = 'luisalejandro/condiment-i386'
    FABRIC_FUNCTION_MAP[CONTAINERS]['generate_condiment_base_image']()


@task
def generate_condiment_base_image_amd64():
    """
    """
    env.arch = 'amd64'
    env.debian_base_image = 'luisalejandro/debian-amd64'
    env.condiment_base_image = 'luisalejandro/condiment-amd64'
    FABRIC_FUNCTION_MAP[CONTAINERS]['generate_condiment_base_image']()
...
@task
def kill_condiment_images():
    """
    """
    FABRIC_FUNCTION_MAP[CONTAINERS]['kill_condiment_images']()


@task
def environment():
    """
    Reproduce the Condiment developer environment.

    This function takes care of downloading and installing the software that
    is needed to develop and maintain Condiment.

    .. versionadded:: 0.2
    """
    FABRIC_FUNCTION_MAP[CONTAINERS]['pull_debian_base_image']()
    FABRIC_FUNCTION_MAP[CONTAINERS]['pull_condiment_base_image']()
...
def django(command):
    ...
    with nested(hide('warnings', 'stderr', 'running'),
        ...
        if command == 'syncdb':
            ...
            run('bash %(condiment_django_syncdb_script)s' % env)

        elif command == 'runserver':

            run('bash %(condiment_django_runserver_script)s' % env)
