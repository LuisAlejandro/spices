...
from condiment.common.errors import CondimentError
...
class BaseFormat(object):
    ...
    def parse_keyvalue_pairs(self, pairs):
        ...
        for kv in pairs:
            if '=' not in kv:
                raise CondimentError('Expected "option=value". Found "%s"' % kv)
            ...
            if v.startswith('@'):
                ...
                try:
                    ...
                    raise CondimentError('No such file or directory: '
                                      '%s (argument:%s)' % (filename, k))
                except Exception, e:
                    raise CondimentError('Bad file %s' % e)
...
class YAMLFormat(BaseFormat):
    ...
    def format(self, data):
        """Format ``data`` in Condiment preferred YAML format."""
...
def get_charm_formatter(charm_format):
    ...
    if charm_format == PythonFormat.charm_format:
        ...
        raise CondimentError('Expected charm format to be either 1 or 2, got %s' %
                          (charm_format,))
